...
$deep = 0;
foreach ($anaTree as $k) if ($k['PID'] > 0) $deep++;
...
foreach ($ana->getNewProducts(null, 1, 50, 'DESC') as $p) {
    ...
    if (! is_array($kat)) $kat = [$kat];
    foreach ($kat as $kid) {
        $kid = (int) $kid;
        if (! isset($anaTree[$kid])) continue;
        // Yolu cikar
        $path = [];
        $cur = $anaTree[$kid];
        while ($cur) {
...
if (! $target) { echo "Sanitize-friendly aday bulunamadi\n"; exit(1); }
...
if (isset($vList['Barkod'])) $vList = [$vList];
...
if ($newId === 0) { echo "FAIL\n"; exit(1); }

sleep(3);
$bUrun = $bayi->getProductByBarcode($vList[0]['Barkod']);
if (! $bUrun) { echo "Bayi'den geri cekilemedi\n"; exit(1); }
